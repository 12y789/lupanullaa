...
exports.handler = async function(event) {
  ...
  if (!apiKey) {
    return { statusCode: 500, headers: CORS, body: JSON.stringify({ content: [{ type: 'text', text: 'Hitilafu ya seva. Wasiliana na admin.' }] }) };
  }
  ...
  try {
    const response = await fetch(url, {
      ...
      body: JSON.stringify({
        system_instruction: {
          parts: [{ text: system || 'Wewe ni msaidizi wa elimu wa Lupanulla Tanzania. Jibu kwa Kiswahili safi na cha kuelimisha.' }]
        },
